...
function DiameterConnection(options, socket) {
    ...
    self.socket.on('data', function(data) {
        try {
            buffer = Buffer.concat([buffer, data instanceof Buffer ? data : new Buffer.from(data)]);
    ...
    self.sendRequest = function(request, timeout) {
        ...
        var promise = deferred.promise.timeout(timeout, 'Request timed out, no response was received in ' + timeout + 'ms. '+request.body);
